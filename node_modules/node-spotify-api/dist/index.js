...
var Spotify = function () {
    ...
    Spotify.prototype.search = function (search, cb) {
        var _this = this;
        var request;
        var opts = {
            method: "GET",
            uri: SEARCH_URI + search.type + "&q=" + search.query + "&limit=" + (search.limit || "20"),
            json: true
        };
    ...
    Spotify.prototype.request = function (query, cb) {
        var _this = this;
    ...
    Spotify.prototype.isTokenExpired = function () {
        if (this.token) {
            if (Date.now() / 1e3 >= this.token.expires_at - 300) {
                return true
            }
        }
        return false
    };
    Spotify.prototype.setToken = function () {
        var _this = this;
        var opts = {
            method: "POST",
            uri: TOKEN_URI,
        ...
        return rp(opts).then(function (token) {
            ...
            return _this.token.expires_at = +expireTime / 1e3 + _this.token.expires_in
        })
    };
    Spotify.prototype.getTokenHeader = function () {
    ...
    Spotify.prototype.getCredentialHeader = function () {
        return {
            Authorization: "Basic " + new Buffer(this.credentials.id + ":" + this.credentials.secret).toString("base64")
        }
    };
    return Spotify
}();
